...
class ReceiptController:
    ...
    def initialize_routes(self):
        ...
        @self.router.get(
            "/one-off_receipt_list", response_model=List[schemas.ReceiptNoItems]
        )
        def get_user_quick_split_receipts(request: Request):
            ...
            if rcts is None:
                raise HTTPException(
                    status_code=404, detail="Unable to get users for receipt. "
                )
            # print(rct)
            return rcts
        ...
        def user_select_items(
            items_data: schemas.GetItems,
            room_code: str,
            receipt_id: int,
            request: Request,
        ):
            ...
            if not self.service.is_user_in_room(usr["id"], room_code):
                raise HTTPException(status_code=404, detail="User is not in room")
            did_add = self.service.user_select_items(
                items_data.item_id_list,
                usr["id"],
                receipt_id,
                items_data.user_total_cost,
                room_code,
            )

            if not did_add:
                raise HTTPException(
                    status_code=500, detail="Error adding user to items"
                )
            return did_add

        @self.router.post("/assign-items/{receipt_id}", response_model=bool)
        def user_select_items(
            items_data: schemas.AssignItems, receipt_id: int, request: Request
        ):
            # Take in user selected items and add to users field in Item object
        ...
        def add_users_to_receipt(
            users_list: List[int], receipt_id: int, request: Request
        ):

            usr = request.state.user

            did_add = self.service.receipt_add_users(
                receipt_id=receipt_id, users_list=users_list
            )

            if not did_add:
                raise HTTPException(
                    status_code=500, detail="Error adding users to receipt"
                )
            return did_add
        ...
        def get_user_items(room_code: str, receipt_id: int, request: Request):
            ...
            if data is None:
                raise HTTPException(
                    status_code=500,
                    detail="Error getting user items - user is not in room code",
                )
        ...
        def create_empty_receipt(request: Request, data: schemas.UploadReceiptData):
            ...
            new_rct = self.service.create_empty_receipt(
        ...
        async def upload_receipt_to_room(
            request: Request,
            receipt_img: UploadFile = File(...),
            data: schemas.UploadReceiptData = Depends(schemas.checker),
        ):
            usr = request.state.user
            ...
            if data.room_code or usr["id"]:
                success = self.service.add_receipt_to_s3(
                    room_code=data.room_code,
                    user_id=usr["id"],
                    file_content=file_content,
                    img_filename=receipt_img.filename,
                )
            else:
                raise HTTPException(
                    status_code=400, detail="invalid room code or user_id"
                )
            ...
            if success:
                ...
                rct_name = (
                    (
                        ...
                        else receipt_dict["merchant_name"]
                    ),
                )
                new_rct = self.service.create_receipt(
                    room_code=data.room_code,
                    receipt_name=rct_name,
        ...
        async def download_receipts_from_room(room_code: str):
            ...
            if not file_contents:
                raise HTTPException(
                    status_code=404, detail="No receipts found for the room"
                )
